...
% Data folders
    root_folder = 'G:\Shared drives\BSL_Data2\HNE_AgingDOE_Processed\HNE_FCC';
    
    cell_folder = {'4CPD 4C (25-39)' ,'             ','          ','4CPD 4C (25-42)';...
                     '4CPD 3C (25-39)' ,'4CPD 3C (25-40)','4CPD 3C (25-41)','           ';...
                     '               ','4CPD 2C (25-40)','4CPD 2C (25-41)','4CPD 2C (25-42)';...
                     '               ','             ','4CPD 1C (25-41)','4CPD 1C (25-42)';
                     '              ','             ','             ','4CPD 0.5C (25-42)'};



% Output space allocation
    [N,M] = size(cell_folder);
...
for n = 1:N
    for m = 1:M
        ...
        filelist = dir([root_folder filesep cell_folder{n,m} filesep '10degC' filesep '*s01*Merged.mat']);

        if ~isempty(filelist)    
            data_load = load([filelist.folder filesep filelist.name]);
            data_merged = data_load.data_merged;

            data_ocv = aging_ocv_fitting(data_merged,ocpn,ocpp,n_points,w_ocv_scale,w_dvdq_scale,N_iter,N_multistart);

            mol.LLI(n,m) = data_ocv(mol.j).LLI;
            mol.LAMp(n,m) = data_ocv(mol.j).LAMp;
            mol.LAMn(n,m) = data_ocv(mol.j).LAMn;
            mol.dQ_LLI(n,m) = data_ocv(mol.j).dQ_LLI;
            mol.dQ_LAMp(n,m) = data_ocv(mol.j).dQ_LAMp;
            mol.dQ_data(n,m) = data_ocv(mol.j).dQ_data;
            
            eol.LLI(n,m) = data_ocv(eol.j).LLI;
            eol.LAMp(n,m) = data_ocv(eol.j).LAMp;
            eol.LAMn(n,m) = data_ocv(eol.j).LAMn;
            eol.dQ_LLI(n,m) = data_ocv(eol.j).dQ_LLI;
            eol.dQ_LAMp(n,m) = data_ocv(eol.j).dQ_LAMp;
            eol.dQ_data(n,m) = data_ocv(eol.j).dQ_data;

            figure(6)
            title(cell_folder{n,m})
            pause(t_pause_plot)
            close all
        end
...
% for each file, fitting all RPTs (OCV), and calculates LLI, LAMp...

% loop over steps & find OCV index
function data_ocv = aging_ocv_fitting(data_merged,ocpn,ocpp,n_points,w_ocv_scale,w_dvdq_scale,N_iter,N_multistart)
    ...
    legend({'Loss by LLI','Loss by LAMp','Loss Data'}); 
